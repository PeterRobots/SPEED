...
class SSIM:
    ...
    def __call__(self, img1, img2):
        ...
        if self.window is None:
            ...
            # Channel is set to 1 since we consider color images_1 as volumetric images_1
        else:
            window = self.window
...
class CalMetrics:
    def __init__(self, device="cuda:0"):
        ...
        self.flolpips = Flolpips().eval().to(device)
        self.lpips.requires_grad_(False)
        self.flolpips.requires_grad_(False)
        self.scaler = 255.0
    ...
    def cal_lpips(self, pred, gt):
        ...
        lpips_value = lpips_value.view(lpips_value.shape[0])
        return lpips_value

    def cal_flolpips(self, pred, gt, i0, i1):
        """ Note: pred and gt have range [0,1]"""
        flolpips_value = self.flolpips(i0, i1, pred, gt)
        flolpips_value = flolpips_value.view(flolpips_value.shape[0])
